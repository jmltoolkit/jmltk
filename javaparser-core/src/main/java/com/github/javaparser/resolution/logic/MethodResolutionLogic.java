...
public class MethodResolutionLogic {
     ...
     */
    public static SymbolReference<ResolvedMethodDeclaration> findMostApplicable(List<ResolvedMethodDeclaration> methods, String name, List<ResolvedType> argumentsTypes, TypeSolver typeSolver, ResolvedReferenceTypeDeclaration invocationContext) {
        SymbolReference<ResolvedMethodDeclaration> res = findMostApplicable(methods, name, argumentsTypes, typeSolver, false, invocationContext);
        if (res.isSolved()) {
            return res;
        }
    ...
    public static SymbolReference<ResolvedMethodDeclaration> findMostApplicable(List<ResolvedMethodDeclaration> methods, String name, List<ResolvedType> argumentsTypes, TypeSolver typeSolver, boolean wildcardTolerance, ResolvedReferenceTypeDeclaration invocationContext) {
        if (invocationContext != null) {
            ...
            for (ResolvedMethodDeclaration method : resolvedMethods) {
                ...
                if (containerType.internalTypes().stream().anyMatch(internalType -> internalType.getQualifiedName().equals(invocationContext.getQualifiedName()))) {
                    // inner classes can see anything in their surrounding class
                    continue;
                } else if (declaration.isPrivate() && !(invocationContext.getQualifiedName().equals(containerType.getQualifiedName()))) {
                    methods.remove(method);
                } else {
                    final boolean samePackage = containerType.getPackageName().equals(invocationContext.getPackageName());
                    if (declaration.isProtected() && !(containerType.isAssignableBy(invocationContext) || samePackage)) {
                        methods.remove(method);
    ...
    public static SymbolReference<ResolvedMethodDeclaration> solveMethodInType(ResolvedTypeDeclaration typeDeclaration, String name, List<ResolvedType> argumentsTypes, ResolvedReferenceTypeDeclaration invocationContext) {
        return solveMethodInType(typeDeclaration, name, argumentsTypes, false, invocationContext);
    }

    // TODO: Replace TypeDeclaration.solveMethod
    public static SymbolReference<ResolvedMethodDeclaration> solveMethodInType(ResolvedTypeDeclaration typeDeclaration, String name, List<ResolvedType> argumentsTypes, boolean staticOnly, ResolvedReferenceTypeDeclaration invocationContext) {
